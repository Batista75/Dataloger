...
class RefossLocalClient:
    """
    Refoss local protocol client using socket broadcast discovery (port 9989).
    Découverte automatique et communication locale sans cloud.
    """
    ...
    def discover_device(self) -> str | None:
        """Découvre un appareil Refoss sur le LAN via socket broadcast."""
        if self._device_ip:
        ...
        try:
            # Envoyer une requête de découverte en broadcast
            discovery_msg = b"discovery"
            sock.sendto(discovery_msg, (self.REFOSS_BROADCAST_ADDR, self.REFOSS_PORT))
    ...
    def read_measurement(self) -> Measurement:
        """
        Lis une mesure depuis l'appareil Refoss local découvert.
        Première tentative: découverte auto si pas encore trouvé.
        """
        ...
        # Strict mode: no simulated values in refoss_local_socket unless explicitly allowed.
    ...
    def _read_mock(self) -> Measurement:
        """Données de test multi-canaux (triphasé) pour validation avant implémentation complète du protocole Refoss."""
        ...
        # Meter 2 with potential solar production (negative)
        ...
        # Separate production (negative) from consumption (positive)
